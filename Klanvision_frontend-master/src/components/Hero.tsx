...
export default function Hero() {
  ...
  return (
    ...
    <section id="hero" className="hero-bg" style={{ minHeight: '100svh', height: 'auto', paddingTop: 'clamp(80px, 12vw, 112px)', paddingBottom: 'clamp(60px, 8vw, 96px)', boxSizing: 'border-box', position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'center' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'clamp(16px, 2.5vw, 40px)', alignItems: 'center' }} className="hero-grid">
          ...
          <motion.div initial={{ x: -60, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ duration: 0.8 }} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <motion.h1
              ...
              style={{ fontSize: 'clamp(1.25rem, 2.8vw, 2.8rem)', lineHeight: 1.15, wordBreak: 'break-word', overflowWrap: 'break-word' }}
            >
              Unique, Safe, and{' '}
              ...
              >
                {"Scalable Digital Innovations".split(" ").map((word, i) => (
                  <motion.span
                    key={i}
      ...
      {/* ── Responsive Media Queries ────────────────────────────────
          Handled globally in index.css for all breakpoints.
          Inline overrides only for hero-specific rules. */}
      <style>{`
        ...
        @media (max-width: 480px) {
          ...
          .badge { font-size: 11px !important; padding: 7px 14px !important; letter-spacing: 1px !important; }
        }
        @media (max-height: 500px) and (orientation: landscape) {
          #hero { min-height: unset !important; padding-top: 80px !important; padding-bottom: 32px !important; }
